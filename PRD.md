...
| **Art Style** | Zelda: Echoes of Wisdom — pastel chibi toy diorama, thick outlines, Nintendo soft palette. **Pure 2D top-down** (orthographic camera). |
...
1. **Explore** — Walk the castle courtyard (and later, the world map) as the Little Lord. Orthographic 2D camera follows the avatar top-down.
...
### 3.1. Player Avatar & Movement  *(post-pivot, updated 2026-04-12 — now pure 2D)*
- **Pure 2D orthographic** — Fully 2D top-down game. Orthographic camera (size 5.5) tracking the player in XY. No 3D geometry, no perspective, no billboard rotation. Camera at z=-10, all sprites at z=0, ground at z=1.
- **Free movement (Stardew / Undertale style)** — Not tile-based. The avatar moves continuously via direct `transform.position += dir * speed * Time.deltaTime` in the XY plane. No NavMesh, no Rigidbody, no CharacterController.
- **4-direction sprite sheets** — Each character has 4 facing directions (N / S / E / W), 2 walk frames each = 8 sprites. Generated by `tools/image_gen/generate_sprites.py` (SDXL pipeline with per-direction prompts + fixed seed per character). Idle = frame 0 of current facing.
- **Input scheme** — Mobile-first:
  - **Mobile (primary)**: Virtual joystick (bottom-left, 260px) for movement. Tap right half of screen to interact when near an NPC.
  - **PC/Steam**: WASD/arrows for movement, `E` to interact, `Esc` for menu.
- **Interaction trigger** — `InteractionFinder` iterates `NPCIdentity.RegisteredNPCs` (self-registration pattern, no physics layers) at 10 Hz. Closest NPC within 2.2m shows a floating prompt. Interact key or tap triggers `DialogueBoxUI.Open(npcId)`.
...
### 3.4. Territory & World Map
- **2D Top-down Castle** — Orthographic sprite-based castle courtyard. Background art from SDXL, characters as sprite overlays.
...
| Rendering | 2D orthographic top-down, SpriteRenderer-based |
| Avatar + NPC movement | Custom transform-based 2D controller (no NavMesh, no Rigidbody) + VirtualJoystick for mobile |
...
| 16 | **Zelda EoW pivot — 2D roaming foundation**: pure 2D orthographic top-down, free-movement player controller, SDXL 4-dir sprite sheets, virtual joystick for mobile, self-registration NPC proximity interact, bottom-screen dialogue box, legacy card UI destroyed at runtime | 🔲 **Next — active pivot** |
...
| 23 | Android/iOS native builds + Final QA (virtual joystick already implemented in M16) | 🔲 Backlog |
...
| SDXL `bg_castle_courtyard` (static PNG) | ✅ Kept — used as 2D ground sprite behind characters |
